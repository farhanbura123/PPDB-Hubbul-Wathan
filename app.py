...
def register():
    if request.method == 'POST':
        ...
        # Buat token JWT dengan menyertakan nama siswa
...
def pendaftaran_siswa():
    ...
    try:
        ...
        return render_template('pendaftaran.html', user=user['namaSiswa'], user_has_submitted=user_has_submitted)
    except jwt.ExpiredSignatureError:
        return redirect(url_for('login'))
    except jwt.InvalidTokenError:
        return redirect(url_for('login'))
...
def riwayat_pendaftaran():
    ...
    try:
        ...
        return render_template('riwayat_daftar.html', pendaftar_list=pendaftar_list, user=user)
    except jwt.ExpiredSignatureError:
        return redirect(url_for('login'))
    except jwt.InvalidTokenError:
        return redirect(url_for('login'))
